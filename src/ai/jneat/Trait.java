 ...
 */
   ...
   */
	  double[] params;
	   ...
	   public Trait(Trait t) {
		 ...
		 this.params = new double[p_num_trait_params];
		 for (int j = 0; j < p_num_trait_params; j++)
			params[j] = t.params[j];
   ...
   */
	   public Trait() {
		 trait_id = 0;
		 params = new double[p_num_trait_params];
		 for (int j = 0; j < p_num_trait_params; j++)
			params[j] = 0.0;
	  } /**
   * Insert the method's description here.
   ...
   */
	   public void mutate()
	  {
		 for(int count = 0; count< p_num_trait_params; count++)
		 {
			if (NeatRoutine.randfloat()> p_trait_param_mut_prob)
			{
			   params[count]+=(NeatRoutine.randposneg()*NeatRoutine.randfloat())* p_trait_mutation_power;
			   if (params[count]<0) 
				  params[count]=0;
			}
		 }
