...
public class Genome extends Neat {
    ...
    public double compatibility(Genome g) {
        ...
        return (p_disjoint_coeff * (num_disjoint / 1.0) + p_excess_coeff * (num_excess / 1.0)
                + p_mutdiff_coeff * (mut_diff_total / num_matching));
	...
	public void mutate_add_sensor(Population pop) {
		...
		for (int i = 0; i < sensors.size(); i++) {
			...
			for (int j = 0; j < genes.size(); j++) {
				...
				if (gene.lnk.in_node == sensor &&
						gene.lnk.out_node.gen_node_label ==
								NeatConstant.OUTPUT)
					outputConnections++;
			}
		...
		NNode sensor = (NNode)
				sensors.elementAt(NeatConstant.myRandom.nextInt(sensors.size()));
		...
		for (int i = 0; i < outputs.size(); i++) {
			...
			if (!found) {
				Iterator innovations = pop.innovations.iterator();
				boolean done = false;

				while (!done) {
					if (!innovations.hasNext()) {
						int traitNum = NeatRoutine.randint(0,
								traits.size()-1);
						...
						double currInnov =
								pop.getCurr_innov_num_and_increment();
						Gene gene = new Gene((Trait)
								traits.elementAt(traitNum),
								newWeight,sensor,output,false,
								currInnov,newWeight);
						genes.add(gene);

						pop.innovations.add(new Innovation(sensor.node_id,
								output.node_id,
								currInnov,
								newWeight,
								traitNum));
						...
						if (innov.innovation_type == NeatConstant.NEWLINK
							...
							Gene gene = new Gene((Trait)
									traits.elementAt(innov.new_traitnum),innov.new_weight,sensor,output,false,innov.
									innovation_num1,
									0);
							genes.add(gene);
    ...
    public boolean mutate_add_link(Population pop, int tries) {
        ...
        if (NeatRoutine.randfloat() < p_recur_only_prob)
            do_recur = true;
        else
            do_recur = false;
    ...
    public Genome(int new_id, int i, int o, int n, int nmax, boolean r, double linkprob) {
        ...
        traits = new Vector(p_num_trait_params, 0);
